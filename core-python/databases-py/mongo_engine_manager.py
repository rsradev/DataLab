...
class Investment(Document):
    ...
    amount = fields.FloatField(min_value=0.0001)
    timestamp = fields.DateTimeField(default=datetime.datetime.now)
    sell = fields.BooleanField(default=False)

    def __str__(self):
        return f'<Investemnt | coin: {self.coin}, currency: {self.currency}, amount: {self.amount}>'
...
@click.command(help="Seed the database with sample data, use --force flag to ignore existing data")
